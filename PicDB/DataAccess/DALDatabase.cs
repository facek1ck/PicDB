...
namespace PicDB.DataAccess
{
    public class DALDatabase : IDAL
    {
        ...
        {
            string query = "INSERT INTO ExifData (Id, Name, TagNumber, Value, Comment) OUTPUT INSERTED.Id " + "VALUES (NEWID(), @name, @tagNumber, @value, @comment)";
        ...
        public void DeletePictureById(Guid ID)
        {
            ...
            connection.Open();
            var exifProps = GetExifPropertiesForPicture(p);
            if(exifProps == null)
            {
                connection.Close();
                return;
